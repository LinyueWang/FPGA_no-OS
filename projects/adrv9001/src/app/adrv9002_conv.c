...
static void adrv9002_axi_tx_test_pattern_set(struct axi_adc *axi_dev, const int off,
...
static void adrv9002_axi_tx_test_pattern_restore(struct axi_adc *axi_dev, const int off,
...
static void adrv9002_axi_rx_test_pattern_pn_sel(struct axi_adc *axi_dev, const int off,
						const adi_adrv9001_SsiType_e ssi_type)
{
	int n_chan = axi_dev->num_channels;
	int c;
...
void adrv9002_axi_digital_tune_verbose(struct adrv9002_rf_phy *phy, uint8_t field[][8], const bool tx,
				       const int channel)
{
}

static int adrv9002_axi_find_point(const uint8_t *field, const uint8_t sz, uint8_t *data_start)
{
	int i = sz;
...
int adrv9002_post_setup(struct adrv9002_rf_phy *phy)
{
	...
	if (!phy->rx2tx2) {
		...
		axi_adc_write(phy->rx1_adc, AIM_AXI_REG(ADI_RX2_REG_OFF, AXI_ADC_REG_CNTRL), R1_MODE);
		axi_adc_write(phy->rx1_adc, AIM_AXI_REG(ADI_TX1_REG_OFF, ADI_TX_REG_CTRL_2), TX_R1_MODE);
		axi_adc_write(phy->rx1_adc, AIM_AXI_REG(ADI_TX2_REG_OFF, ADI_TX_REG_CTRL_2), TX_R1_MODE);
	}
